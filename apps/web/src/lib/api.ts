...
export const api = {
  ...
  lineAccounts: {
    ...
    update: (id: string, data: Partial<Pick<LineAccount, 'name' | 'channelAccessToken' | 'channelSecret' | 'isActive'>> & { loginChannelId?: string | null; loginChannelSecret?: string | null; liffId?: string | null }) =>
      fetchApi<ApiResponse<LineAccount>>(`/api/line-accounts/${id}`, {
        method: 'PUT',
        body: JSON.stringify(data),
      }),
